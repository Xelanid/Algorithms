...
struct tree {
    ...
    tree(int key) : value(key), count(1), h(1), left(nullptr), parent(nullptr), right(nullptr) {}
};

tree* root = nullptr;

tree* find(int key) {
    tree* temp = root, *parent = nullptr;
    while (temp) {
        parent = temp;
...
tree* remove(tree* cur, int key){
    ...
    if (cur->value > key) {
        ...
        if (cur->right) {
            ...
            cur->right = remove(cur->right, temp->value);
            if (cur->right)
                cur->right->parent = cur;
        } else if (cur->left) {
            temp = cur->left;
            while (temp->right)
                temp = temp->right;
            cur->value = temp->value;
            cur->left = remove(cur->left, temp->value);
            if (cur->left)
                cur->left->parent = cur;
        }
        else {
